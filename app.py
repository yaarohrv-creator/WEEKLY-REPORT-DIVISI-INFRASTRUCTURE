...
def get_connection():
    ...
    # Tambahkan kolom waktu_input jika belum ada
    try:
        cursor.execute("ALTER TABLE laporan_mingguan ADD COLUMN waktu_input DATETIME DEFAULT CURRENT_TIMESTAMP")
...
# Pastikan koneksi dan tabel dibuat di awal jalannya program
conn = get_connection()
...
if menu == "Dashboard Progress":
    ...
    # Penanganan membaca database dengan aman di Streamlit Cloud
    try:
    ...
    if df_view.empty:
        df_view = pd.DataFrame(columns=[
            'No', 'real_id', 'Waktu Input', 'Nama Kontraktor', 
            'Unit Proyek', 'Minggu Ke-', 'Progress (%)', 'Catatan Pekerjaan'
        ])
    ...
    # Fungsi Membuat File Excel dengan Styling dan Foto
    def generate_excel():
        output = io.BytesIO()
